...
def normalize_coverage(value):
    ...
    allowed = {"completa": "Completa", "parcial": "Parcial", "no cubierta": "No cubierta", "fuera de alcance": "Fuera de alcance"}
    return allowed.get(v, safe_text(value, "Pendiente"))


def normalize_validation_method(value):
    v = safe_text(value).strip().lower()
    mapping = {"ui": "UI", "interfaz": "UI", "interfaz de usuario": "UI", "bd": "BD", "base de datos": "BD", "database": "BD", "api": "API", "web service": "API", "web services": "API", "mixta": "Mixta", "mixto": "Mixta"}
    return mapping.get(v, safe_text(value, "Pendiente"))


def _remove_trailing_pipe(value):
    return re.sub(r"\s*\|\s*$", "", safe_text(value)).rstrip()


_DESCRIPTION_LABELS = ("Producto", "Módulo", "Descripción", "Resultado esperado de la prueba", "Precondiciones", "Caso de uso relacionado")
...
def _extract_labeled_blocks(value):
    ...
    for index, match in enumerate(matches):
        ...
        content = re.sub(r"\*\*\s*$", "", text[start:end].strip()).strip()
...
def _extract_description_block(value, label, next_labels):
    ...
    if not text:
        return ""
    labels_pattern = "|".join(re.escape(item) for item in [label, *next_labels])
...
def _clean_description_content(value):
    ...
    return re.sub(r"(?is)^\s*\**Descripción\**\s*:\s*", "", text, count=1).strip()


def build_azure_description(product, module, description, expected, preconditions, related_use_case):
    raw_values = {"Producto": _remove_trailing_pipe(product), "Módulo": _remove_trailing_pipe(module), "Descripción": _remove_trailing_pipe(description), "Resultado esperado de la prueba": _remove_trailing_pipe(expected), "Precondiciones": _remove_trailing_pipe(preconditions), "Caso de uso relacionado": _remove_trailing_pipe(related_use_case)}
    combined_blocks = {}
    for value in raw_values.values():
        for key, content in _extract_labeled_blocks(value).items():
            if content and not combined_blocks.get(key):
                combined_blocks[key] = content
    values = {label: combined_blocks.get(label, value) for label, value in raw_values.items()}
...
def _domain_prefix(source_content="", hu_text="", module=""):
    """AU por defecto; HO si el módulo identifica explícitamente Hogar."""
    module_text = safe_text(module).strip().casefold()
    if re.search(r"\bhogar\b", module_text):
        return "HO"
    evidence = " ".join(safe_text(x) for x in (source_content, hu_text)).casefold()
    hogar_patterns = (r"\bproducto\s*[:\-]?\s*hogar\b", r"\bmódulo\s*[:\-]?\s*hogar\b", r"\bmodule\s*[:\-]?\s*hogar\b", r"\bseguro\s+de\s+hogar\b", r"\bseguros?\s+de\s+hogar\b", r"\bcotizador(?:es)?\s+de\s+hogar\b", r"\bcotizador(?:es)?\s+hogar\b", r"\bproducto\s+hogar\b", r"\bmódulo\s+hogar\b", r"\bfuncionalidad\s+de\s+hogar\b")
