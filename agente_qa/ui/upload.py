"""Componente Streamlit para carga y procesamiento de documentación QA."""
from __future__ import annotations

import streamlit as st


def render_upload(extract_source):
    """Renderiza carga/vista previa y devuelve el texto fuente procesado."""
    st.subheader("📁 Carga de Documento")
    st.info("Formatos de HU: TXT, MD, PDF, DOCX, XLSX/CSV. Para PDF escaneado se requiere OCR; esta versión no inventa texto que no pueda extraer.")
    uploaded = st.file_uploader("Arrastra o selecciona un documento", type=["txt", "md", "pdf", "docx", "xlsx", "xls", "csv"])
    ...
    if uploaded:
        ...
        source_text = st.text_area("✏️ O ingresa el texto manualmente", height=220, placeholder="Pega aquí la Historia de Usuario o documentación fuente...")
        if source_text:
            st.session_state.source_content = source_text
    if source_text:
